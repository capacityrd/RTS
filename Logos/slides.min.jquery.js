  ...
  $.fn.slides = function( option ) {
    ...
    return this.each(function(){
      ...
      function checkLoop() {
        ...
        if((current - 1) === - 1){ // revisa sí estás en la primer posición
          elem.find($('.' + option.prev)).fadeOut();
          elem.find($('.' + option.next)).fadeIn();

        } else if((current + 1) === total ){  // revisa sí estás en la última posición
          elem.find($('.' + option.next)).fadeOut();
          elem.find($('.' + option.prev)).fadeIn();

        } else { // Si no se cumple nada de lo anterior, haz tu rutina normal
      ...
      // click handling
      $('a.linkx', elem).on('click', function(){
        // pause slideshow
        if (option.play) {
           pause();
        }
